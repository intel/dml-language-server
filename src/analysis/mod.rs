...
impl Hash for DMLError {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        match self.severity {
            ...
            Some(severity) => panic!(
                "Cannot hash unsupported diagnostic severity: {:?}", severity),
        }
...
fn for_each_struct_type(ty: &DMLType, visited: &mut HashSet<ZeroSpan>,
    ...
    let Some(FieldContainer::Struct(st)) =
        DeviceAnalysis::peel_to_field_container(ty, Some(type_storage)) else {
        return;
    };
    ...
    for (_, member_ty) in &st.members {
        for_each_struct_type(member_ty, visited, type_storage, visit);
    }
}
...
impl DeviceAnalysis {
    ...
    fn peel_to_field_container(ty: &DMLType,
                               type_storage: Option<&GlobalTypeStorage>)
        -> Option<FieldContainer> {
        let mut cur: DMLType = ty.clone();
        let mut visited_typedefs = HashSet::new();
        loop {
            let concrete = cur?;
            cur = match concrete.as_ref() {
                DMLConcreteType::StructType(st) =>
                    return Some(FieldContainer::Struct(st.clone())),
                DMLConcreteType::Layout(l) =>
                    return Some(FieldContainer::Struct(l.base.clone())),
                DMLConcreteType::Trait(t) =>
                    return Some(FieldContainer::Trait(t.clone())),
                DMLConcreteType::Typedef(td) if td.underlying.is_none() => {
                    if !visited_typedefs.insert(td.decl_name.span) {
                        return None;
                    }
                    type_storage?.get_underlying(&td.decl_name.val)?.clone()
                }
    ...
    pub fn fields_of_type_symbol(&self, type_sym: &SymbolRef)
        ...
        match Self::peel_to_field_container(ty, Some(&self.type_storage))? {
            FieldContainer::Struct(st) =>
    ...
    fn resolve_struct_field_in_type(&self,
        ...
        match node {
            NodeRef::Simple(simple) => {
                let Some(container) = Self::peel_to_field_container(
                    ty, Some(&self.type_storage)) else { return; };
...
fn extend_with_struct_fields(maker: &SymbolMaker, symbols: &mut SymbolStorage,
    ...
    for ty in type_storage.resolved_types() {
        for_each_struct_type(ty, &mut visited, type_storage, &mut |st| {
...
fn coalesce_trait_member_references(symbols: &SymbolStorage) {
    let concrete_symbols: Vec<SymbolRef> = symbols.param_symbols.values()
        .flat_map(|by_parent| by_parent.values().cloned())
        .chain(symbols.variable_symbols.values().cloned())
        .collect();

    for trait_symbol in symbols.trait_member_symbols.values()
        .flat_map(|members| members.values()) {
        let member_span = trait_symbol.lock().unwrap().loc;
        let matching_concrete: Vec<SymbolRef> = concrete_symbols.iter()
            .filter(|symbol| {
                let symbol = symbol.lock().unwrap();
                symbol.bases.contains(&member_span)
                    || symbol.declarations.contains(&member_span)
            })
            .cloned()
            .collect();
        if matching_concrete.is_empty() {
            continue;
        }

        let mut references = trait_symbol.lock().unwrap().references.clone();
        for symbol in &matching_concrete {
            references.extend(symbol.lock().unwrap().references.iter().copied());
        }
        trait_symbol.lock().unwrap().references = references.clone();
        for symbol in matching_concrete {
            symbol.lock().unwrap().references = references.clone();
        }
    }
}
...
fn add_new_symbol_from_shallow(maker: &SymbolMaker,
    ...
    let mut object_source = shallow.clone();
    if let DMLShallowObjectVariant::Parameter(param) = &shallow.variant {
        let unresolved = param.declarations.iter()
            .chain(param.used_definitions.iter())
            .find_map(|(_, declaration)| declaration.typed.as_ref());
        if let Some(unresolved) = unresolved {
            object_source.resolved_type = Some(
                eval_type_simple(unresolved, types, errors));
        }
    }
    let source = SymbolSource::DMLObject(
        DMLObject::ShallowObject(Box::new(object_source)));
    let new_sym = symbol_ref!(
        maker,
        *shallow.location(),
        shallow.kind(),
...
impl DeviceAnalysis {
    ...
    pub fn new(root: IsolatedAnalysis,
        ...
        device.match_references(&bases,
                                ...
                                &mut errors,
                                &status)?;
        coalesce_trait_member_references(&device.symbol_info);
        ...
        for error in errors {
            if !unique_errors.insert(error.clone()) {
                continue;
            }
...
mod error_identity_tests {
    ...
    fn error(severity: Option<DiagnosticSeverity>) -> DMLError {
        DMLError {
            span: ZeroSpan::invalid("error.dml"),
            description: "description".to_string(),
            severity,
            related: vec![(ZeroSpan::invalid("related.dml"), "related".to_string())],
        }
    }

    fn hash(error: &DMLError) -> u64 {
    ...
    #[test]
    fn diagnostic_identity_includes_severity_and_related_information() {
        let original = error(Some(DiagnosticSeverity::ERROR));
        assert_eq!(original, original.clone());
        assert_eq!(hash(&original), hash(&original.clone()));

        let mut different_severity = original.clone();
        different_severity.severity = Some(DiagnosticSeverity::WARNING);
        assert_ne!(original, different_severity);
        assert_ne!(hash(&original), hash(&different_severity));

        let mut different_related = original.clone();
        different_related.related[0].1 = "different related information".to_string();
        assert_ne!(original, different_related);
        assert_ne!(hash(&original), hash(&different_related));
    }

    #[test]
    fn diagnostic_without_severity_has_stable_identity() {
