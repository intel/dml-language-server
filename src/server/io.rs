...
pub trait Output: Sync + Send + Clone + 'static {
    ...
    #[inline]
    fn failure(&self, id: Value, error: RpcError) {
        let resp = Response {
            result: None,
            error: Some(error),
            id,
            jsonrpc: Some(JSONRPC_V2.to_owned()),
        };
        self.response(serde_json::to_string(&resp).unwrap());
    }

    /// Notifies the client of a standard failure, with custom message.
    #[inline]
    fn custom_failure<M>(&self, id: RequestId, code: StandardError, msg: Option<M>)
    where M: ToString {
        let mut error = standard_error(code, None);
        if let Some(message) = msg {
            error.message = message.to_string();
        }
        self.failure(Value::from(id), error);
    }

    /// Notifies the client of a standard failure, with custom message.
    #[inline]
    fn failure_message<M>(&self, id: RequestId, code: Value, msg: M)
    where M: ToString {
        if let Some(code) = code.as_i64() {
            let error = RpcError {
                code: code as i32, message: msg.to_string(), data: None };
            self.failure(Value::from(id), error);
        } else {
            self.custom_failure(id, StandardError::InternalError, Some(msg));
        }
    }
