...
pub struct SpanData {
    ...
    pub span: crate::span::Span<crate::span::ZeroIndexed>,
...
impl VfsSpan {
    pub fn from_usv(span: crate::span::Span<crate::span::ZeroIndexed>, len: Option<u64>) -> VfsSpan {
        VfsSpan::UnicodeScalarValue(SpanData { span, len })
    }

    pub fn from_utf16(span: crate::span::Span<crate::span::ZeroIndexed>, len: Option<u64>) -> VfsSpan {
        VfsSpan::Utf16CodeUnit(SpanData { span, len })
    }

    /// Return a UTF-8 byte offset in `s` for a given text unit offset.
    pub fn byte_in_str(&self, s: &str, c: crate::span::Column<crate::span::ZeroIndexed>) -> Result<usize, Error> {
        match self {
    ...
    pub fn span(&self) -> &crate::span::Span<crate::span::ZeroIndexed> {
        &self.as_inner().span
    }
...
impl Change {
    fn file(&self) -> PathBuf {
        match *self {
            Change::AddFile { ref file, .. } => file.to_path_buf(),
            Change::ReplaceText { ref span, .. } => span.span().path(),
        }
    }
}
...
impl<U> Vfs<U> {
    ...
    pub fn snapshot_file(&self, path: &Path) -> Result<TextFile, Error> {
    ...
    pub fn load_line(
        &self,
        path: &Path,
        line: crate::span::Row<crate::span::ZeroIndexed>,
    ) -> Result<String, Error> {
    ...
    pub fn load_lines(
        &self,
        path: &Path,
        line_start: crate::span::Row<crate::span::ZeroIndexed>,
        line_end: crate::span::Row<crate::span::ZeroIndexed>,
    ) -> Result<String, Error> {
        self.0.load_lines(path, line_start, line_end)
    }

    pub fn load_span(&self, span: crate::span::Span<crate::span::ZeroIndexed>) -> Result<String, Error> {
...
impl<T: FileLoader, U> VfsInternal<T, U> {
    ...
    fn on_changes(&self, changes: &[Change]) -> Result<(), Error> {
        ...
        for (file_name, changes) in coalesce_changes(changes) {
            {
                let mut files = self.files.lock().unwrap();
                if let Some(file) = files.get_mut(file_name.as_path()) {
            ...
            let mut file = T::read(file_name.as_path())?;
            file.make_change(&changes)?;
    ...
    fn load_line(&self, path: &Path, line: crate::span::Row<crate::span::ZeroIndexed>) -> Result<String, Error> {
    ...
    fn load_span(&self, span: crate::span::Span<crate::span::ZeroIndexed>)
                 -> Result<String, Error> {
        self.ensure_file(&span.path(),
    ...
    fn snapshot_file(&self, path: &Path) -> Result<TextFile, Error> {
        self.ensure_file(path, |f| match &f.kind {
            FileKind::Text(textfile) => Ok(textfile.clone()),
            _ => Err(Error::BadFileKind),
        })
    }
...
fn coalesce_changes(changes: &'_ [Change]) -> HashMap<PathBuf, Vec<&'_ Change>> {
...
impl FileContents {
    pub fn as_string_content(&self) -> Result<String, Error> {
        match &self {
            FileContents::Text(content) => Ok(content.to_string()),
            FileContents::Binary(_) => Err(Error::BadFileKind),
        }
    }
}
...
impl<U> File<U> {
    ...
    fn load_line(&self, line: crate::span::Row<crate::span::ZeroIndexed>) -> Result<&str, Error> {
        match self.kind {
    ...
    pub fn load_range(&self,
                      range: crate::span::Range<crate::span::ZeroIndexed>)
                      -> Result<&str, Error> {
        match self.kind {
...
impl TextFile {
                        ...
                        let byte_end = if let Some(len) = len {
                            ...
                            byte_start
                                + vfs_span.byte_in_str(
                                    ...
                                    crate::span::Column::new_zero_indexed(len as u32),
                                )? as u32
                        } else {
    ...
    pub fn load_range(&self, range: crate::span::Range<crate::span::ZeroIndexed>) -> Result<&str, Error> {
...
pub struct ParseTextFileError;

impl FromStr for TextFile {
    type Err = ParseTextFileError;
...
fn byte_in_str(s: &str, c: crate::span::Column<crate::span::ZeroIndexed>) -> Result<usize, Error> {
...
fn byte_in_str_utf16(s: &str, c: crate::span::Column<crate::span::ZeroIndexed>) -> Result<usize, Error> {
    let (mut utf8_offset, mut utf16_offset) = (0, 0);
    let target_utf16_offset = c.0 as usize;

    for chr in s.chars().chain(std::iter::once('\0')) {
        match utf16_offset.cmp(&target_utf16_offset) {
            Ordering::Greater => break,
            Ordering::Equal => return Ok(utf8_offset),
            Ordering::Less => (),
        }
