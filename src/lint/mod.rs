...
fn obtain_lint_annotations(file: &str) -> (Vec<DMLStyleError>,
    ...
    for (row, line) in file.lines().enumerate() {
        ...
        if let Some(capture) = LINT_ANNOTATION.captures(line) {
            ...
            let Some(target) = RuleType::from_str(target_capture.as_str())
            else {
                incorrect_annotations.push(DMLStyleError {
                    error: LocalDMLError {
                        range: ZeroRange::from_u32(
    ...
    if !unapplied_annotations.is_empty() {
        // TODO: the range of this warning could be improved
        incorrect_annotations.push(DMLStyleError {
            error: LocalDMLError {
                range: ZeroRange::from_u32(
                    last_line as u32,
    ...
    */
    ...
    fn test_annotation_parse() {
        ...
        assert_eq!(simplified_errs,
                   vec![(3 as u32, "Invalid lint rule target 'unknown_rule'.".to_string()),
                        (4 as u32, "Invalid command 'configure' in dls-lint \
                                    annotation.".to_string()),
                        (7 as u32, "dls-lint annotations without effect at \
