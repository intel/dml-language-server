...
static FUNCALL_BROKEN_AFTER_PARENT_INCORRECT_FIRST_LINE: &str = "
method effect() {
    callback(
               0xABC,
        identifier,
...
";
#[test]
fn funcall_broken_after_paren_incorrect_first_line() {
    let rules = set_up();
    let expected_errors = define_expected_errors!(
        RuleType::LL6,
        (3, 3, 15, 20)
    );
    assert_snippet(FUNCALL_BROKEN_AFTER_PARENT_INCORRECT_FIRST_LINE, expected_errors, &rules);
}
